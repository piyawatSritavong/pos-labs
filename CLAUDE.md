...
- Deleting a part that any document references **archives** it (`is_active = false` on `part_master` and all its `address_master` rows) — a hard delete would cascade the vehicle's stock row away. Archiving strands whatever qty sat on those rows — invisible to both the vehicle stock page and the till — so move the stock to the surviving product first; [deploy/supabase-merge-duplicate-parts.sql](deploy/supabase-merge-duplicate-parts.sql) is the worked example. Earlier deletes left 2,206 pieces sitting on archived rows across the two vans and the warehouse; the shop has chosen to keep them rather than write them off, so a hidden quantity will come back as real stock the first time a purchase order is received against one of those products — that is deliberate, not a bug to "fix" by zeroing them. Archived parts are out of the restock catalog and the inbound/transfer pickers (the purchase-order picker has an opt-in, because receiving is the only thing that reactivates a part), but their barcodes still resolve — a scan of one answers `part_archived`.
