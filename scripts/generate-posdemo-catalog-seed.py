...
def build_seed(products: list[dict[str, object]], source_name: str) -> str:
    lines = [
        ...
        "  (code, bar_code, unit_id, name, name_th, receipt_name, details, cost, price, min_price, image, is_active)",
        "VALUES",
    ]
    part_values = []
        ...
        part_values.append(
            ...
            + ", ".join(
                [
                    ...
                    sql(item["cost"]),
                    sql(item["price"]),
                    sql((item["price"] * Decimal("0.90")).quantize(Decimal("0.01"))),
                    "''",
                    "true",
                ]
            )
    ...
    lines.extend(
        [
            ...
            "   details, cost, price, min_price, image, is_active)",
            ...
            "SELECT code, part_code, store_id, shelf, qty, rop, remarks, true",
