...
echo.
echo --- NULL counts (should ALL be 0 — non-zero = data needs repair; rerun load-real-data.bat) ---
"%PSQL_EXE%" -h %DB_HOST% -p %DB_PORT% -U %DB_USER% -d %DB_NAME% -t -A -F " | " -c "SELECT 'parts.bar_code NULL'  AS t, count(*) FROM part_master    WHERE bar_code IS NULL UNION ALL SELECT 'parts.details NULL',          count(*) FROM part_master    WHERE details  IS NULL UNION ALL SELECT 'parts.cost NULL',             count(*) FROM part_master    WHERE cost     IS NULL UNION ALL SELECT 'addresses.shelf NULL',        count(*) FROM address_master WHERE shelf    IS NULL UNION ALL SELECT 'addresses.max NULL',          count(*) FROM address_master WHERE \"max\"   IS NULL UNION ALL SELECT 'addresses.remarks NULL',      count(*) FROM address_master WHERE remarks  IS NULL ORDER BY t;"
